    ...
    <div className="bg-white rounded-2xl p-6 shadow-md border border-slate-200 space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
        ...
        <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-200">
          <div className="text-[11px] font-bold text-emerald-800 uppercase tracking-wider">{t('now_serving')}</div>
          ...
          <div className="text-[10px] text-emerald-600 font-medium mt-1">{t('counter1')}</div>
        </div>

        <div className="bg-amber-50 p-4 rounded-xl border border-amber-200">
          <div className="text-[11px] font-bold text-amber-800 uppercase tracking-wider">{t('your_token')}</div>
        ...
        <div className="bg-blue-50 p-4 rounded-xl border border-blue-200">
          <div className="text-[11px] font-bold text-blue-800 uppercase tracking-wider">{t('farmers_ahead')}</div>
          <div className="text-3xl font-black text-blue-700 mt-1">{farmersAhead}</div>
          <div className="text-[10px] text-blue-600 font-medium mt-1">{t('in_queue_line')}</div>
        </div>

        <div className="bg-purple-50 p-4 rounded-xl border border-purple-200">
          <div className="text-[11px] font-bold text-purple-800 uppercase tracking-wider">{t('est_wait_time')}</div>
      ...
      <div>
        <div className="flex justify-between text-xs font-semibold text-slate-600 mb-1">
          <span>{t('queue_progress')}</span>
          <span>{farmersAhead === 0 ? t('turn_now') : `${farmersAhead} ${t('farmers_ahead_count')}`}</span>
        </div>
      ...
      <div>
        ...
        <div className="flex items-center space-x-3 overflow-x-auto pb-4 pt-2 px-1">
          {tokensList.map((tokenVal, idx) => {
            const isCompleted = tokenVal < nowServing;
            const isServing = tokenVal === nowServing;
            const isUser = tokenVal === userToken;

            return (
              <React.Fragment key={tokenVal}>
                <div
                ...
                >
                  <span className="text-[10px] font-semibold uppercase">
                    {isServing ? t('serving') : isUser ? t('you') : isCompleted ? t('done') : t('wait')}
                  </span>
                  <span className="text-base font-black">#{tokenVal}</span>
                  {isServing && <span className="text-[9px] bg-emerald-700 px-1 rounded mt-0.5">{t('counter1')}</span>}
                </div>
