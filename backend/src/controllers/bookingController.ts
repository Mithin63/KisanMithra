...
export const createBooking = (req: Request, res: Response) => {
  ...
  const farmer = mockStore.farmers.find(f => f.id === farmer_id || f.user_id === farmer_id);
  const centre = mockStore.centres.find(c => c.id === centre_id);
  const crop = mockStore.crops.find(cr => cr.id === crop_id);
  ...
  const centreBookingsToday = mockStore.bookings.filter(b => b.centre_id === centre_id && b.booking_date === booking_date);
  const nextTokenNumber = 120 + centreBookingsToday.length + 1;

  const newBookingId = mockStore.bookings.length + 1;
  const newBooking = {
    id: newBookingId,
    farmer_id: farmer.id,
    centre_id,
  ...
  const waitingBookings = mockStore.bookings.filter(b => b.centre_id === centre_id && (b.status === 'WAITING' || b.status === 'ARRIVED'));
  const queuePos = waitingBookings.length + 1;
  const estWait = smartQueueService.calculateWaitingTime(queuePos, centre.avg_processing_mins, centre.active_counters);

  const newToken = {
    id: mockStore.queueTokens.length + 1,
  ...
  mockStore.queueTokens.push(newToken);
...
export const getBookingById = (req: Request, res: Response) => {
  ...
  const crop = mockStore.crops.find(cr => cr.id === booking.crop_id);
  const queueToken = mockStore.queueTokens.find(qt => qt.booking_id === booking.id);
  const procurement = mockStore.procurementRecords.find(p => p.booking_id === booking.id);
  const payment = procurement ? mockStore.payments.find(pm => pm.procurement_id === procurement.id) : null;
...
export const getFarmerBookings = (req: Request, res: Response) => {
  const farmerId = parseInt(req.params.farmerId);
  const farmerBookings = mockStore.bookings.filter(b => b.farmer_id === farmerId);

  const hydrated = farmerBookings.map(b => {
    const centre = mockStore.centres.find(c => c.id === b.centre_id);
    const crop = mockStore.crops.find(cr => cr.id === b.crop_id);
    const queue = mockStore.queueTokens.find(qt => qt.booking_id === b.id);
    const procurement = mockStore.procurementRecords.find(pr => pr.booking_id === b.id);
