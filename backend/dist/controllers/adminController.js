...
exports.getAdminCentresList = exports.getAdminStatistics = void 0;
const mockStore_1 = require("../store/mockStore");
const getAdminStatistics = (req, res) => {
    const totalFarmers = 12845 + mockStore_1.mockStore.farmers.length;
    const todayBookings = 1284 + mockStore_1.mockStore.bookings.length;
    const procurementCompleted = 932 + mockStore_1.mockStore.procurementRecords.length;
    ...
    const centreQueueChart = mockStore_1.mockStore.centres.map(c => {
        const centreBookings = mockStore_1.mockStore.bookings.filter(b => b.centre_id === c.id);
...
const getAdminCentresList = (req, res) => {
    const centresMatrix = mockStore_1.mockStore.centres.map(c => {
        const centreBookings = mockStore_1.mockStore.bookings.filter(b => b.centre_id === c.id);
        const waiting = centreBookings.filter(b => b.status === 'WAITING' || b.status === 'ARRIVED');
...
exports.getAdminCentresList = getAdminCentresList;
